...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
    return <div data-slot="sheet-header" className={clsx("flex flex-col gap-0.5 p-4", className)} {...props} />;
}
...
function SheetTitle({ className, ...props }: SheetPrimitive.Title.Props) {
    return (
        <SheetPrimitive.Title
            ...
            className={clsx("text-foreground text-base font-medium", className)}
            {...props}
        />
    );
}
